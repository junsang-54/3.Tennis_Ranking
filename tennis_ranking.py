...
tennis_NoNan = (tennis.dropna(thresh=12)) #column의 데이터가 없는(nan값을 가진) row 제거
tennis_NoNan
true_copy_tennis_NoNan = tennis_NoNan.copy() #정제된 데이터 copy


#nan 제거 검증
true_copy_tennis_NoNan.isnull().sum() #nan값이 있는지 isnull함수로 확인하고, column별로 nan값의 갯수를 한눈에 보기 편하게 sum함수 사용

true_copy_tennis_NoNan.describe() #데이터의 정보 요약
